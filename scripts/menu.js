...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function openCartModal() {
        ...
        shoppingCart.forEach((item) =>
            modalItems.appendChild(createCartItem(item))
        );
        displayTotalItems();
        displayTotalPrice();
        showModal();
    ...
    function displayTotalItems() {
        const totalItemsElement = createHTMLElement(
            "div",
            `Items: ${calculateTotalQuantity(shoppingCart)}`,
            "total-items"
        );
    ...
    function updateTotalPriceElement(formattedPrice) {
        ...
        const totalPriceElement = createHTMLElement(
            "div",
    ...
    function createOrderButton(item) {
        const orderButton = createHTMLElement("button", "Order Now", "", [
            "order-button",
        ]);
    ...
    function createCartItem(item) {
        ...
        const btnCountContainer = createHTMLElement(
            "div",
            "",
            "btn-count-container"
        );
        const quantitySpan = createHTMLElement(
            "span",
            item.quantity,
            "total-item-count"
        );
